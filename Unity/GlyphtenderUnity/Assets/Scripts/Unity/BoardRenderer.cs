...
namespace Glyphtender.Unity
{
    ...
    public class BoardRenderer : MonoBehaviour
    {
        ...
        private char _ghostTileLetter; // Letter of the ghost tile
        private Player _ghostTileOwner; // Owner of the ghost tile
        ...
        private void CreateTile(HexCoord coord, Tile tile)
        {
            ...
            if (tilePrefab != null)
            {
                ...
                float tileSize = hexSize * glyphlingSize;
            ...
            else
            {
                ...
                tileObj.transform.localScale = new Vector3(hexSize * glyphlingSize, 0.1f, hexSize * glyphlingSize);
        ...
        public void ShowGhostTile(HexCoord position, char letter, Player owner, GameObject existingObject = null)
        {
            ...
            _ghostTileLetter = letter;
            _ghostTileOwner = owner;
            ...
            if (existingObject != null)
            {
                ...
                // Apply runeblossom material (even for external objects - ensures correct texture on board)
                var renderer = _ghostTile.GetComponent<Renderer>();
                if (renderer != null)
                {
                    Material mat = SpriteLoader.Instance?.GetRuneblossomMaterial(letter, owner) ?? GetPlayerMaterial(owner);
                    if (mat != null)
                    {
                        renderer.material = mat;
                        Debug.Log($"[BoardRenderer] Applied runeblossom material for '{letter}' owned by {owner}");
                    }
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    _ghostTile.transform.localScale = new Vector3(hexSize * glyphlingSize, 0.1f, hexSize * glyphlingSize);
                    _ghostTile.transform.SetParent(transform);
                }
        ...
        private void OnGameRestarted()
        {
            ...
            // If game ended mid-placement, ghost objects would persist into next game.
            if (_ghostGlyphling != null)
            {
                Destroy(_ghostGlyphling);
                _ghostGlyphling = null;
